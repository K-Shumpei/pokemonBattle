...
function isSuccess(pokemon) {
    ...
    for (const damage of pokemon.damage) {
        ...
        if (target === false)
            continue;
        if (target.item.isName('ねらいのまと') === true)
            continue;
        if (pokemon.move.selected.isStatus() && pokemon.move.selected.name === 'でんじは')
            continue;
    ...
    for (const damage of pokemon.damage) {
        ...
        if (target === false)
            continue;
        if (pokemon.move.selected.getFlag().powder === false)
            continue;
    ...
    for (const damage of pokemon.damage) {
        ...
        if (fieldStatus.weather.isRainy(target)) {
            if (pokemon.move.selected.name === 'かみなり')
                continue;
            if (pokemon.move.selected.name === 'ぼうふう')
                continue;
        }
        if (fieldStatus.weather.isSnowy()) {
            if (pokemon.move.selected.name === 'ふぶき')
                continue;
        }
...
function getCompatibility(pokemon, target) {
    const atkType = pokemon.move.selected.type;
    const defType = getPokemonType(target);
    let result = 1.0;
...
function isActionFailure(pokemon) {
    ...
    sleep: if (pokemon.statusAilment.isAsleep()) {
        ...
        if (pokemon.statusAilment.turn > 0) {
            ...
            if (sleepingMoveList.includes(pokemon.move.selected.name) === true)
                break sleep;
            return true;
        }
        else {
    ...
    focusPunch: if (pokemon.move.selected.name === 'きあいパンチ') {
    ...
    disable: if (pokemon.stateChange.disable.isTrue === true) {
        if (pokemon.stateChange.disable.text !== pokemon.move.selected.name)
            break disable;
        writeLog(`${getArticle(pokemon)}は かなしばりで 技が 出せない!`);
        return true;
    }
    gravity: if (fieldStatus.whole.gravity.isTrue === true) {
        if (pokemon.move.selected.getFlag().gravity === false)
            break gravity;
    ...
    healBlock: if (pokemon.stateChange.healBlock.isTrue === true) {
        if (pokemon.move.selected.getFlag().heal === false)
            break healBlock;
        if (pokemon.move.selected.name === 'かふんだんご' && pokemon.damage[0].trainer !== pokemon.trainer)
            break healBlock;
        writeLog(`${getArticle(pokemon)}は かいふくふうじで 技が 出せない!`);
        return true;
    }
    throatChop: if (pokemon.stateChange.throatChop.isTrue === true) {
        if (pokemon.move.selected.getFlag().sound === false)
            break throatChop;
        writeLog(`${getArticle(pokemon)}は じごくづきの効果で 技が 出せない!`);
        return true;
    }
    taunt: if (pokemon.stateChange.taunt.isTrue === true) {
        if (pokemon.move.selected.name === 'さきどり')
            break taunt;
        if (!pokemon.move.selected.isStatus())
            break taunt;
...
function changeMoveType(pokemon) {
    ...
    if (pokemon.move.selected.name === 'だいちのはどう') {
        if (isGrounded(pokemon) === true && fieldStatus.terrain.isElectric())
            pokemon.move.selected.type = 'ELECTRIC';
        if (isGrounded(pokemon) === true && fieldStatus.terrain.isGrassy())
            pokemon.move.selected.type = 'GRASS';
        if (isGrounded(pokemon) === true && fieldStatus.terrain.isPsychic())
            pokemon.move.selected.type = 'PSYCHIC';
        if (isGrounded(pokemon) === true && fieldStatus.terrain.isMisty())
...
function failureByMoveSpec(pokemon) {
    ...
    steelRoller: if (pokemon.move.selected.name === 'アイアンローラー') {
    ...
    hyperspaceFury: if (pokemon.move.selected.name === 'いじげんラッシュ') {
    ...
    darkVoid: if (pokemon.move.selected.name === 'ダークホール') {
    ...
    auraWheel: if (pokemon.move.selected.name === 'オーラぐるま') {
    ...
    auroraVeil: if (pokemon.move.selected.name === 'オーロラベール') {
        if (!fieldStatus.weather.isSnowy())
            break auroraVeil;
        pokemon.damage = [];
        pokemon.declareFailure();
    ...
    stockpile: if (pokemon.move.selected.name === 'たくわえる') {
    ...
    lastResort: if (pokemon.move.selected.name === 'とっておき') {
        let isFailure = false;
        // 「とっておき」を覚えていない
    ...
    spitUp: if (pokemon.move.selected.name === 'はきだす' || pokemon.move.selected.name === 'のみこむ') {
    ...
    stuffCheeks: if (pokemon.move.selected.name === 'ほおばる') {
        for (const berry of berryTable) {
            if (berry.name === pokemon.item.name)
                break stuffCheeks;
        }
        pokemon.damage = [];
        pokemon.declareFailure();
    ...
    fling: if (pokemon.move.selected.name === 'なげつける') {
        ...
        if (pokemon.item.name === null)
            break fling;
        if (pokemon.item.isName(pokemon.item.name) === false)
            break fling;
        // 不適格な持ち物である
        if (pokemon.item.name === 'べにいろのたま')
            break fling;
        if (pokemon.item.name === 'あいいろのたま')
            break fling;
        if (pokemon.item.name === 'くちたけん')
            break fling;
        if (pokemon.item.name === 'くちたたて')
            break fling;
        if (pokemon.item.name === 'だいこんごうだま')
            break fling;
        if (pokemon.item.name === 'だいしらたま')
            break fling;
        if (pokemon.item.name === 'だいはっきんだま')
            break fling;
        if (pokemon.name === 'ギラティナ(アナザー)' && pokemon.item.name === 'はっきんだま')
            break fling;
        if (pokemon.name === 'ギラティナ(オリジン)' && pokemon.item.name === 'はっきんだま')
            break fling;
        ...
        if (megaStoneTable.filter(mega => mega.name === pokemon.item.name && mega.name === pokemon.name).length === 1)
            break fling;
        if (paradoxPokemonList.includes(pokemon.name) && pokemon.item.name === 'ブーストエナジー')
            break fling;
        pokemon.damage = [];
        pokemon.declareFailure();
        return true;
    ...
    naturalGift: if (pokemon.move.selected.name === 'しぜんのめぐみ') {
        ...
        if (pokemon.item.isName(pokemon.item.name) === false)
            break naturalGift;
        // 不適格な持ち物である
        if (berryTable.filter(berry => berry.name === pokemon.item.name).length !== 1)
            break naturalGift;
        pokemon.damage = [];
        pokemon.declareFailure();
        return true;
    }
    fakeOut: if (pokemon.move.selected.name === 'ねこだまし' || pokemon.move.selected.name === 'であいがしら' || pokemon.move.selected.name === 'たたみがえし') {
        pokemon.damage = [];
        pokemon.declareFailure();
        return true;
    }
    noRetreat: if (pokemon.move.selected.name === 'はいすいのじん') {
    ...
    suckerPunch: if (pokemon.move.selected.name === 'ふいうち') {
        pokemon.damage = [];
        pokemon.declareFailure();
        return true;
    }
    poltergeist: if (pokemon.move.selected.name === 'ポルターガイスト') {
    ...
    protect: if (protectMoveList.includes(pokemon.move.selected.name)) {
    ...
    snore: if (pokemon.move.selected.name === 'いびき' || pokemon.move.selected.name === 'ねごと') {
    ...
    lowKick: if (pokemon.move.selected.name === 'けたぐり' || pokemon.move.selected.name === 'くさむすび' || pokemon.move.selected.name === 'ヘビーボンバー' || pokemon.move.selected.name === 'ヒートスタンプ') {
...
function failureByAbility(pokemon) {
    const targetList = getTargetList(pokemon);
    const one = targetList[0];
    damp: if (explosionMoveList.includes(pokemon.move.selected.name) === true) {
...
function effectAlwaysActivate(pokemon) {
    ...
    rage: if (pokemon.move.selected.name === 'いかり') {
...
function abilityChangeType(pokemon) {
    ...
    protean: if (pokemon.ability.isName('へんげんじざい') || pokemon.ability.isName('リベロ')) {
        if (myType.length === 1 && myType[0] === pokemon.move.selected.type)
            break protean;
...
function preliminaryAction(pokemon) {
    const targetList = getTargetList(pokemon);
    const one = targetList[0];
    if (pokemon.move.selected.getFlag().charge === false)
        return false;
...
function dicideMagnitudePower(pokemon) {
    if (pokemon.move.selected.name !== 'マグニチュード')
        return;
    const random = getRandom();
    if (random >= 95) {
...
function disableByConcealment(pokemon) {
    ...
    for (const target of targetList) {
        ...
        if (pokemon.ability.isName('ノーガード'))
            continue;
        if (target.target.ability.isName('ノーガード'))
            continue;
        if (pokemon.move.selected.name === 'どくどく' && getPokemonType(pokemon).includes('POISON'))
            continue;
        if (pokemon.move.selected.name === 'アロマセラピー')
            continue;
        if (pokemon.move.selected.name === 'いやしのすず')
            continue;
        if (pokemon.move.selected.name === 'てだすけ')
            continue;
        let isValid = true;
        if (target.target.stateChange.dig.isTrue === true) {
            if (pokemon.move.selected.name === 'じしん')
                isValid = true;
            if (pokemon.move.selected.name === 'マグニチュード')
                isValid = true;
        }
        if (target.target.stateChange.fly.isTrue === true) {
            if (pokemon.move.selected.name === 'かぜおこし')
                isValid = true;
            if (pokemon.move.selected.name === 'たつまき')
                isValid = true;
            if (pokemon.move.selected.name === 'かみなり')
                isValid = true;
            if (pokemon.move.selected.name === 'スカイアッパー')
                isValid = true;
            if (pokemon.move.selected.name === 'うちおとす')
                isValid = true;
            if (pokemon.move.selected.name === 'ぼうふう')
                isValid = true;
            if (pokemon.move.selected.name === 'サウザンアロー')
                isValid = true;
        }
...
function disableByOtherProtect(pokemon) {
    ...
    for (const target of targetList) {
        quickGuard: if (fieldStatus.getSide(target.target.trainer).quickGuard.isTrue === true) {
            if (pokemon.move.selected.priority <= 0)
                break quickGuard;
            if (pokemon.ability.isName('ふかしのこぶし') && pokemon.move.selected.getFlag().contact === true)
                break quickGuard;
            target.damage.success = false;
        ...
        wideGuard: if (fieldStatus.getSide(target.target.trainer).wideGuard.isTrue === true) {
            if (pokemon.move.selected.target !== '相手全体' && pokemon.move.selected.target !== '自分以外')
                break wideGuard;
            if (pokemon.ability.isName('ふかしのこぶし') && pokemon.move.selected.getFlag().contact === true)
                break wideGuard;
            target.damage.success = false;
        ...
        craftyShield: if (fieldStatus.getSide(target.target.trainer).craftyShield.isTrue === true) {
            ...
            if (!pokemon.move.selected.isStatus())
                break craftyShield;
            if (pokemon.move.selected.target === '全体')
                break craftyShield;
            if (pokemon.move.selected.target === '味方全体')
                break craftyShield;
            if (pokemon.move.selected.name === 'コーチング')
                break craftyShield;
            if (pokemon.move.selected.name === 'オウムがえし')
                break craftyShield;
            if (pokemon.move.selected.name === 'さきどり')
                break craftyShield;
            target.damage.success = false;
...
function disableByProtect(pokemon) {
    ...
    for (const target of targetList) {
        if (target.target.stateChange.protect.isTrue === false)
            continue;
        if (pokemon.move.selected.getFlag().protect === false)
            continue;
        if (pokemon.ability.isName('ふかしのこぶし') && pokemon.move.selected.getFlag().contact === true)
            continue;
        if (target.target.stateChange.protect.text === 'キングシールド' && pokemon.move.selected.isStatus())
            continue;
        if (target.target.stateChange.protect.text === 'ブロッキング' && pokemon.move.selected.isStatus())
            continue;
        target.damage.success = false;
...
function disableByMaxGuard(pokemon) {
    ...
    for (const target of targetList) {
        if (target.target.stateChange.protect.text !== 'ダイウォール')
            continue;
        if (notMaxGuardMoveList.includes(pokemon.move.selected.name) === false && pokemon.move.selected.getFlag().protect === false)
            continue;
        target.damage.success = false;
        writeLog(`${getArticle(target.target)}は 攻撃から 身を守った!`);
    }
...
function failureByTelekinesis(pokemon) {
    ...
    if (pokemon.move.selected.name !== 'テレキネシス')
        return false;
    if (one.target.name === 'ディグダ')
        isFailure = true;
    if (one.target.name === 'ダグトリオ')
        isFailure = true;
    if (one.target.name === 'スナバァ')
        isFailure = true;
    if (one.target.name === 'シロデスナ')
        isFailure = true;
    if (one.target.name === 'メガゲンガー')
        isFailure = true;
