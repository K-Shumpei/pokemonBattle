...
function isActionFailure(pokemon) {
    ...
    const sleep = (pokemon) => {
        if (!pokemon.statusAilment.isAsleep())
            return false;
        const turn = (pokemon.ability.isName('Early Bird')) ? 2 : 1; // 特性「はやおき」
    ...
    const frozen = (pokemon) => {
        ...
        if (pokemon.move.selected.getMaster().defrost) {
            if (pokemon.move.selected.name !== 'Burn Up')
    ...
    const focusPunch = (pokemon) => {
        if (pokemon.move.selected.name !== 'Focus Punch')
    ...
    const flinch = (pokemon) => {
        if (!pokemon.stateChange.flinch.isTrue)
            return false;
        pokemon.msgFlinch();
        if (!pokemon.ability.isName('Steadfast'))
            return true; // 特性「ふくつのこころ」
...
function changeMoveType(pokemon) {
    ...
    galvanize: if (pokemon.ability.isName('Galvanize')) { // 特性「エレキスキン」
        pokemon.move.selected.activateSkin('Electric');
    }
    aerilate: if (pokemon.ability.isName('Aerilate')) { // 特性「スカイスキン」
        pokemon.move.selected.activateSkin('Flying');
    }
    normalize: if (pokemon.ability.isName('Normalize')) { // 特性「ノーマルスキン」
        pokemon.move.selected.activateSkin('Normal');
    }
    pixilate: if (pokemon.ability.isName('Pixilate')) { // 特性「フェアリースキン」
        pokemon.move.selected.activateSkin('Fairy');
    }
    refrigerate: if (pokemon.ability.isName('Refrigerate')) { // 特性「フリーズスキン」
...
function deductPowerPoint(pokemon) {
    ...
    const pressureSide = (_a = main.getPokemonInSide(!pokemon.isMine())) === null || _a === void 0 ? void 0 : _a.filter(p => p.ability.isName('Pressure')).length;
    ...
    switch (pokemon.move.selected.target) {
        case 'users-field':
            ...
            break;
        case 'opponents-field':
            if (pokemon.move.selected.name === 'Sticky Web')
...
function failureByMoveSpec(pokemon) {
    const steelRoller = (pokemon) => {
        if (pokemon.move.selected.name !== 'Steel Roller')
            return false;
    ...
    const hyperspaceFury = (pokemon) => {
        if (pokemon.move.selected.name !== 'Hyperspace Fury')
            return false;
    ...
    const darkVoid = (pokemon) => {
        if (pokemon.move.selected.name !== 'Dark Void')
            return false;
    ...
    const auraWheel = (pokemon) => {
        if (pokemon.move.selected.name !== 'Aura Wheel')
            return false;
    ...
    const auroraVeil = (pokemon) => {
        if (pokemon.move.selected.name !== 'Aurora Veil')
            return false;
        if (!main.field.weather.isSnowy())
            return false;
        pokemon.attack.reset();
    ...
    const clangorousSoul = (pokemon) => {
        if (pokemon.move.selected.name !== 'Clangorous Soul')
            return false;
    ...
    const stockpile = (pokemon) => {
        if (pokemon.move.selected.name !== 'Stockpile')
            return false;
    ...
    const teleport = (pokemon) => {
        if (pokemon.move.selected.name !== 'Teleport')
            return false;
    ...
    const lastResort = (pokemon) => {
        if (pokemon.move.selected.name !== 'Last Resort')
            return false;
        let isFailure = false;
        // 「とっておき」を覚えていない
    ...
    const spitUp = (pokemon) => {
        if (pokemon.move.selected.name !== 'Spit Up'
            && pokemon.move.selected.name !== 'Swallow')
            return false;
    ...
    const stuffCheeks = (pokemon) => {
        if (pokemon.move.selected.name !== 'Stuff Cheeks')
            return false;
    ...
    const fakeOut = (pokemon) => {
        if (pokemon.move.selected.name !== 'Fake Out'
            ...
            && pokemon.move.selected.name !== 'Mat Block')
            return false;
        pokemon.attack.reset();
        pokemon.msgDeclareFailure();
        return true;
    ...
    const noRetreat = (pokemon) => {
        if (pokemon.move.selected.name !== 'No Retreat')
            return false;
    ...
    const suckerPunch = (pokemon) => {
        if (pokemon.move.selected.name !== 'Sucker Punch')
            return false;
        pokemon.attack.reset();
        pokemon.msgDeclareFailure();
        return true;
    ...
    const poltergeist = (pokemon) => {
        if (pokemon.move.selected.name !== 'Poltergeist')
            return false;
    ...
    const snore = (pokemon) => {
        if (pokemon.move.selected.name !== 'Snore'
            && pokemon.move.selected.name !== 'Sleep Talk')
            return false;
        if (pokemon.statusAilment.isAsleep())
            return false;
        if (pokemon.ability.isName('Comatose'))
    ...
    const rest = (pokemon) => {
        if (pokemon.move.selected.name !== 'Rest')
            return false;
        if (pokemon.status.hp.value.isMax())
            return false;
        if (pokemon.statusAilment.isAsleep())
            return false;
        if (pokemon.ability.isName('Insomnia'))
            return false; // 特性「ふみん」
        if (pokemon.ability.isName('Vital Spirit'))
            return false; // 特性「やるき」
        if (pokemon.ability.isName('Comatose'))
            return false; // 特性「ぜったいねむり」
        pokemon.attack.reset();
    ...
    const lowKick = (pokemon) => {
        if (pokemon.move.selected.name !== 'Low Kick'
            ...
            && pokemon.move.selected.name !== 'Heat Crash')
            return false;
        const attack = pokemon.attack.getValidTarget();
        if (attack.length === 0)
            return false;
...
function failureByAbility(pokemon) {
    damp: if (pokemon.move.selected.getAddOn().explosion) {
        if (!main.isExistAbility('Damp'))
            break damp; // 特性「しめりけ」
...
function effectAlwaysActivate(pokemon) {
    const futureSight = (pokemon) => {
        if (pokemon.move.selected.name !== 'Future Sight'
            && pokemon.move.selected.name !== 'Doom Desire')
            return false;
    ...
    const rage = (pokemon) => {
        if (pokemon.move.selected.name !== 'Rage')
            return false;
...
function disableByConcealment(pokemon) {
    for (const attack of pokemon.attack.getTargetToPokemon()) {
        ...
        if (pokemon.stateChange.lockOn.isTrue)
            continue;
        if (pokemon.ability.isName('No Guard'))
            continue; // 特性「ノーガード」
        if (target.ability.isName('No Guard'))
            continue; // 特性「ノーガード」
        if (pokemon.move.selected.name === 'Toxic' && pokemon.type.has('Poison'))
            continue; // 技「どくどく」
        if (pokemon.move.selected.name === 'Aromatherapy')
            continue; // 技「アロマセラピー」
        if (pokemon.move.selected.name === 'Heal Bell')
            continue; // 技「いやしのすず」
        if (pokemon.move.selected.name === 'Helping Hand')
            continue; // 技「てだすけ」
        let isValid = true;
        if (target.stateChange.dig.isTrue) {
            if (pokemon.move.selected.name === 'Earthquake')
                isValid = true; // 技「じしん」
            if (pokemon.move.selected.name === 'Magnitude')
                isValid = true; // 技「マグニチュード」
        }
        if (target.stateChange.fly.isTrue) {
            if (pokemon.move.selected.name === 'Gust')
                isValid = true; // 技「かぜおこし」
            if (pokemon.move.selected.name === 'Twister')
                isValid = true; // 技「たつまき」
            if (pokemon.move.selected.name === 'Thunder')
                isValid = true; // 技「かみなり」
            if (pokemon.move.selected.name === 'Sky Uppercut')
                isValid = true; // 技「スカイアッパー」
            if (pokemon.move.selected.name === 'Smack Down')
                isValid = true; // 技「うちおとす」
            if (pokemon.move.selected.name === 'Hurricane')
                isValid = true; // 技「ぼうふう」
            if (pokemon.move.selected.name === 'Thousand Arrows')
                isValid = true; // 技「サウザンアロー」
        }
        if (target.stateChange.dive.isTrue) {
            if (pokemon.move.selected.name === 'Surf')
                isValid = true; // 技「なみのり」
            if (pokemon.move.selected.name === 'Whirlpool')
                isValid = true; // 技「うずしお」
        }
        if (isValid)
            continue;
    ...
    for (const attack of pokemon.attack.getTargetToPokemon()) {
        ...
        craftyShield: if (main.field.getSide(target.isMine()).craftyShield.isTrue) {
            ...
            if (pokemon.move.selected.target === 'user-and-allies')
                break craftyShield;
            if (pokemon.move.selected.name === 'Coaching')
                break craftyShield; // 技「コーチング」
            if (pokemon.move.selected.name === 'Mirror Move')
    ...
    for (const attack of pokemon.attack.getTargetToPokemon()) {
        ...
        spikyShield: if (target.stateChange.protect.text === 'ニードルガード') {
            if (!pokemon.move.selected.getMaster().contact)
                break spikyShield;
            if (pokemon.move.selected.name === 'Sky Drop')
                break spikyShield; // 技「フリーフォール」
            if (pokemon.ability.isName('Magic Guard'))
                break spikyShield; // 特性「マジックガード」
...
function failureByTelekinesis(pokemon) {
    if (pokemon.move.selected.name !== 'Telekinesis')
        return false; // 技「テレキネシス」
    for (const attack of pokemon.attack.getTargetToPokemon()) {
        const target = main.getPokemonByBattle(attack);
        const check = (target) => {
            if (target.name === 'Diglett')
                return false; // ディグダ
            if (target.name === 'Dugtrio')
                return false; // ダグトリオ
            if (target.name === 'Sandygast')
                return false; // スナバァ
            if (target.name === 'Palossand')
                return false; // シロデスナ
            if (target.name === 'Gengar Mega')
                return false; // メガゲンガー
...
function disableByCompatibility(pokemon) {
    for (const attack of pokemon.attack.getTargetToPokemon()) {
        const target = main.getPokemonByBattle(attack);
        if (pokemon.move.selected.isStatus() && pokemon.move.selected.name !== 'Thunder Wave')
            continue; // 技「でんじは」
...
function disableGroundMove1st(pokemon) {
    for (const attack of pokemon.attack.getTargetToPokemon()) {
        const target = main.getPokemonByBattle(attack);
        if (!target.ability.isName('Levitate'))
            continue; // 特性「ふゆう」
...
function disableBySubstitute(pokemon) {
    const isSubstitute = (pokemon, target) => {
        if (!target.stateChange.substitute.isTrue)
            return false;
        if (pokemon.move.selected.name === 'Hyperspace Hole')
            return false; // 技「いじげんホール」
        if (pokemon.move.selected.name === 'Hyperspace Fury')
            return false; // 技「いじげんラッシュ」
        if (pokemon.move.selected.name === 'Spectral Thief')
            return false; // 技「シャドースチール」
        if (pokemon.move.selected.isStatus()) {
...
function disableByHitJudgment(pokemon) {
    ...
    const isHit = (pokemon, target) => {
        ...
        if (main.field.weather.isSnowy()) {
            if (pokemon.move.selected.name === 'Blizzard')
                return true; // 技「ふぶき」
        }
...
function disableByMoveSpec3rd(pokemon) {
    const isDisableForAbility = (pokemon, target) => {
        ...
        if (pokemon.move.selected.name === 'Simple Beam') { // 技「シンプルビーム」
            if (target.ability.isName('Simple'))
                return true; // 特性「たんじゅん」
    ...
    const isDisableForItem = (pokemon, target) => {
        ...
        if (pokemon.move.selected.name === 'Corrosive Gas') { // 技「ふしょくガス」
            if (target.name === 'Giratina Origin' && target.item.isName('はっきんだま'))
                return true; // ギラティナ(オリジン)
            if (target.name === 'Giratina Altered' && target.item.isName('はっきんだま'))
                return true; // ギラティナ(アナザー)
            ...
            if (target.name === 'Zacian' && target.item.isName('くちたけん'))
                return true; // ザシアン
            if (target.name === 'Zacian Crowned' && target.item.isName('くちたけん'))
                return true; // ザシアン(王)
            if (target.name === 'Zamazenta' && target.item.isName('くちたたて'))
                return true; // ザマゼンタ
            if (target.name === 'Zamazenta Crowned' && target.item.isName('くちたたて'))
                return true; // ザマゼンタ(王)
        }
