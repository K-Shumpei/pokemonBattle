...
// ダメージが発生したときの効果
function effectsWhenDamageOccurs(pokemon, target) {
    rage: if (target.stateChange.rage.isTrue === true) {
        if (target.damage.substitute === true)
            break rage;
    ...
    // 効果バツグンの技を受けた時
    effective: if (target.damage.effective > 1) {
        if (target.status.remainingHP === 0)
            break effective;
        if (target.damage.substitute === true)
            break effective;
...
function isValidToTargetAdditionalEffect(pokemon, target) {
    if (target.status.remainingHP === 0)
        return false;
    if (target.damage.substitute === true)
        return false;
...
function activateOtherAdditionalEffect(pokemon, target) {
    if (pokemon.moveUsed.name === 'アンカーショット' || pokemon.moveUsed.name === 'かげぬい') {
        if (pokemon.stateChange.sheerForce.isTrue === true)
            return;
        if (isValidToTargetAdditionalEffect(pokemon, target) === false)
            return;
        if (getPokemonType(target).includes('ゴースト') === false)
            return;
        if (target.stateChange.cannotEscape.isTrue === false)
            return;
    ...
    if (pokemon.moveUsed.name === 'うたかたのアリア') {
        ...
        if (isValidToTargetAdditionalEffect(pokemon, target) === false)
            return;
        if (target.status.statusAilment.name !== 'やけど')
            return;
