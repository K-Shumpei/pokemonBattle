...
function registerAllRandom() {
    for (let i = 0; i < 6; i++) {
        ...
        registrationPokemon();
        for (let j = 0; j < 4; j++) {
            if (moveLearnedByPomeon.some(_move => _move.nameEN === pokemon.nameEN) === false)
                continue;
...
// 受信したコマンドの記録
function setSelectedMove(pokemon) {
