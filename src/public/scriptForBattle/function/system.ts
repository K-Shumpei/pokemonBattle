// システム的に一意な順番
function sortByHostParty( pokeList: Pokemon[] ): Pokemon[] {
...
function sortByEachLeft( pokeList: Pokemon[] ): Pokemon[] {
  const result = pokeList.sort( ( a, b ) => {
    // バトル場の並び順（左側が先頭になるように）
    if ( a.order.battle === null ) return -1;
    if ( b.order.battle === null ) return -1;
    if ( a.order.host && a.order.battle < b.order.battle ) return -1;
    if ( !a.order.host && a.order.battle < b.order.battle ) return 1;
    if ( a.order.battle < b.order.battle ) return -1;
    if ( a.order.battle > b.order.battle ) return 1;
    // トレーナー
    if ( a.order.host ) return -1;
    else return 1;
...
function sortByBattle( pokeList: Pokemon[] ): Pokemon[] {
  const result = pokeList.sort( ( a, b ) => {
    // バトル場の並び順（左側が先頭になるように）
    if ( a.order.battle === null ) return -1;
    if ( b.order.battle === null ) return -1;
...
function sortByCottonDown( pokeList: Pokemon[] ): Pokemon[] {
  const result = pokeList.sort( ( a, b ) => {
    // 自分、相手の順番
...
function sortByActionOrder( pokeList: Pokemon[] ): Pokemon[] {
  const result = pokeList.sort( (a, b) => {
    // 技の効果
    if ( a.actionOrder.moveEffect > b.actionOrder.moveEffect ) return -1;
    if ( a.actionOrder.moveEffect < b.actionOrder.moveEffect ) return 1;
    // 優先度
    if ( a.move.selected.priority > b.move.selected.priority ) return -1;
    if ( a.move.selected.priority < b.move.selected.priority ) return 1;
    // 先攻
    if ( a.actionOrder.itemAbilityEffect > b.actionOrder.itemAbilityEffect ) return -1;
    if ( a.actionOrder.itemAbilityEffect < b.actionOrder.itemAbilityEffect ) return 1;
    // 素早さ
...
function sortBySpeed( pokeList: Pokemon[] ): Pokemon[] {
  const result = pokeList.sort( (a, b) => {
    // 素早さ
    if ( a.status.spe.av > b.status.spe.av ) return -1;
    if ( a.status.spe.av < b.status.spe.av ) return 1;
    // 乱数
    if ( a.status.spe.random > b.status.spe.random ) return -1;
    else return 1;
...
function sortByOriginalSpeed( pokeList: Pokemon[] ): Pokemon[] {
  const result = pokeList.sort( (a, b) => {
    // 素早さ
    if ( a.status.spe.avOrg > b.status.spe.avOrg ) return -1;
    if ( a.status.spe.avOrg < b.status.spe.avOrg ) return 1;
    // 乱数
    if ( a.status.spe.random > b.status.spe.random ) return -1;
    else return 1;
...
function sortPokemonList( pokeList: Pokemon[], sort: SortCategory ): Pokemon[] {
  switch ( sort ) {
    case 'host-party':
...
function getPokemonInBattlefield( sort: SortCategory ): Pokemon[] {
  const me: Pokemon[] = main.me.pokemon.filter( p => p.order.battle !== null );
  const opp: Pokemon[] = main.opp.pokemon.filter( p => p.order.battle !== null );
  const result: Pokemon[] = me.concat( opp );

  return sortPokemonList( result, sort );
}

function getPokemonInSide( isMe: boolean ): Pokemon[] {
  const pokemon: Pokemon[] = main.getParty( isMe );
  return pokemon.filter( p => p.order.battle !== null );

  //return sortPokemonList( result, sort );
}

function getPokemonScheduledToAttack(): Pokemon[] {
  const pokeList: Pokemon[] = getPokemonInBattlefield( 'actionOrder' );
  return pokeList.filter( p => p.command.isAttack() );
}

function getPokemonScheduledToExchange(): Pokemon[] {
  const pokeList: Pokemon[] = getPokemonInBattlefield( 'actionOrder' );
  return pokeList.filter( p => p.command.isExchange() );
}

function getPokemonOnLanding( sort: SortCategory ): Pokemon[] {
  const pokeList: Pokemon[] = getPokemonInBattlefield( sort );
